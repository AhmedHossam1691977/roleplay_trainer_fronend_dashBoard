...
export default function AllScenario({ searchTerm, filters }) {
  ...
  const fetchScenarios = useCallback(async (role, keyword = "", activeFilters = {}, page = 1) => {
    try {
      ...
      // فلترة للمستخدم العادي: يرى فقط السيناريوهات غير المحذوفة والمفعلة
  ...
  const toggleStatus = async (scenarioId, currentDeletedStatus) => {
    ...
    try {
      ...
      if (res.ok) {
        // تحديث محلي فوري: نعكس isDeleted ونعكس status
  ...
  const canStartTraining = (scenario) => {
    ...
    // التحقق من الاشتراك إذا كان أدمن
    if (user?.role === 'admin' && user?.subscriptions?.length === 0) return false;
    // السيناريو يجب أن يكون status: true وغير محذوف
        ...
        <>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {scenarios.map((scenario) => {
              ...
              // الكارت يعتبر "نشط" بصرياً فقط إذا كان status: true وغير محذوف
              const isActiveVisual = scenario.status === true && scenario.isDeleted === false;
              
              return (
                <div key={scenario._id} className={`group bg-white rounded-3xl border border-gray-200 shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col overflow-hidden ${!isActiveVisual ? 'opacity-60 grayscale-[0.8]' : ''}`}>
                  ...
                  <div className="px-6 py-4 bg-gray-50/80 border-t border-gray-100 flex gap-2 items-center">
                    <button onClick={() => setSelectedScenario(scenario)} className="p-3 bg-white border border-gray-200 text-gray-500 rounded-2xl hover:bg-blue-50 hover:text-blue-600 transition-all active:scale-90 shadow-sm">
                      <Info size={20} />
                    </button>

                    {(user?.role === 'admin' || user?.role === 'super-admin') && (
                      <>
                        <button onClick={() => { setUpdatingScenarioId(scenario._id); setIsUpdateModalOpen(true); }} className="p-3 bg-white border border-amber-200 text-amber-600 rounded-2xl hover:bg-amber-50 transition-all active:scale-90 shadow-sm">
                          <Edit3 size={20} />
                        </button>
                        {/* استخدام toggleStatus مع isDeleted للتحكم في الأيقونة واللون */}
                        <button 
                          onClick={() => toggleStatus(scenario._id, scenario.isDeleted)} 
                          disabled={togglingId === scenario._id} 
                          className={`p-3 border rounded-2xl transition-all active:scale-90 shadow-sm ${!scenario.isDeleted ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-gray-200 border-gray-300 text-gray-600'}`}
                        >
                          {togglingId === scenario._id ? (
                            <Loader2 size={20} className="animate-spin" />
                          ) : (
                            !scenario.isDeleted ? <ToggleRight size={20} /> : <ToggleLeft size={20} />
                          )}
                        </button>
                      </>
                    )}
                    ...
                    <button 
                      ...
                      className="flex-1 bg-blue-600 text-white py-4 px-4 rounded-2xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-blue-700 transition-all active:scale-95 shadow-lg shadow-blue-100 disabled:bg-gray-400 disabled:shadow-none disabled:cursor-not-allowed"
                    >
                      {user?.role === 'admin' && (!user?.subscriptions || user?.subscriptions?.length === 0) ? 'Upgrade to Start' : 'Start Training'} 
                      <ArrowRight size={16} />
                    </button>
                  </div>
                </div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-2 mt-12 pb-8">
              ...
              {[...Array(totalPages)].map((_, index) => {
                ...
                return (
                  <button key={pageNum} onClick={() => handlePageChange(pageNum)} className={`w-10 h-10 rounded-xl font-bold text-sm transition-all border ${currentPage === pageNum ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-white text-gray-600 border-gray-200 hover:border-blue-300'}`}>
                    {pageNum}
                  </button>
                );
              })}
